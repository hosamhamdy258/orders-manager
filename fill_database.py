...
from orderApp.models import MenuItem, Order, OrderItem, OrderRoom, Restaurant
...
# # Create Groups
all_groups = set(OrderRoom.objects.values_list("name", flat=True))
for group in groups - all_groups:
    OrderRoom.objects.create(name=group, room_number=f"g{time.time_ns()}")
...
for user, group in users_orders - all_order_with_users:
    Order.objects.create(fk_user=UserModel.objects.get(username=user), fk_group=OrderRoom.objects.get(name=group), finished_ordering=True)
